...
function nav(id) {
    ...
    const targetItem = navItems.find(item => item.getAttribute('onclick').includes(`'${id}'`));
    if (targetItem) {
...
function getChatReply(msg) {
    ...
    if (m.includes('yolo') || m.includes('detect') || m.includes('vision')) return chatKnowledge.yolov8;
    if (m.includes('mqtt') || m.includes('http') || m.includes('protocol')) return chatKnowledge.mqtt;
    if (m.includes('esp32') || m.includes('arduino') || m.includes('sensor')) return chatKnowledge.esp32;
    if (m.includes('raspberry') || m.includes('rpi') || m.includes('edge')) return chatKnowledge.raspberrypi;
    if (m.includes('smart city') || m.includes('city') || m.includes('ទីក្រុង')) return chatKnowledge.smartcity;
    if (m.includes('ai') || m.includes('ml') || m.includes('machine') || m.includes('ការ') || m.includes('vision')) return chatKnowledge.ai;
...
function drawBanana(ctx, x, y, scale, angle, detected = false) {
    ...
    ctx.beginPath();
    ctx.moveTo(45, -64); ctx.lineTo(44, -58);
    ctx.moveTo(48, -65); ctx.lineTo(47, -57);
    ctx.moveTo(50, -61); ctx.lineTo(50, -56);
    ...
    // Sugar spots/specks
    let freckles = [
        { x: 32, y: -20, r: 0.9 }, { x: 34, y: -17, r: 1.3 }, { x: 35, y: -22, r: 0.6 },
        { x: 37, y: -12, r: 1.6 }, { x: 31, y: -8, r: 0.9 }, { x: 27, y: -6, r: 0.8 },
        { x: 34, y: -4, r: 1.2 }, { x: 21, y: -1, r: 0.5 }, { x: 24, y: -4, r: 0.8 },
        { x: -12, y: 15, r: 0.7 }, { x: -32, y: 22, r: 0.6 }, { x: 6, y: 11, r: 0.8 }
    ];
...
function toggleInference() {
    ...
    if (isInference) {
        ...
        addLog("[SYSTEM] Inference Live Engine Started (YOLOv8 Endpoint).", "text-blue-400");
    } else {
...
setInterval(() => {
    ...
    fetch('/api/stats')
        ...
        .then(data => {
            ...
            if (document.getElementById('apiStatsBanana')) document.getElementById('apiStatsBanana').innerText = data.banana || 0;
            if (document.getElementById('apiStatsApple')) document.getElementById('apiStatsApple').innerText = data.apple || 0;
            if (document.getElementById('apiStatsOrange')) document.getElementById('apiStatsOrange').innerText = data.orange || 0;
            if (document.getElementById('apiStatsTotal')) document.getElementById('apiStatsTotal').innerText = data.total || 0;

            if (document.getElementById('apiStatus')) document.getElementById('apiStatus').style.background = '#3fb950'; // Green
        })
        .catch(err => {
            if (document.getElementById('apiStatus')) document.getElementById('apiStatus').style.background = '#e84c4c'; // Red
        });
}, 1000);
...
function renderDetection() {
    ...
    detCtx.lineWidth = 1;
    for (let i = 0; i < W; i += 40) { detCtx.beginPath(); detCtx.moveTo(i, 0); detCtx.lineTo(i, H); detCtx.stroke(); }
    for (let j = 0; j < H; j += 40) { detCtx.beginPath(); detCtx.moveTo(0, j); detCtx.lineTo(W, j); detCtx.stroke(); }
    ...
    detectionFruits.forEach(obj => {
        ...
        if (isInference) {
            ...
            if (conf >= confThreshold) {
                ...
                const label = obj.type === 'banana' ? '🍌 ចេក (Banana)' : '🍎 ប៉ោម (Apple)';
                ...
                detCtx.fillText(`${label} ${Math.floor(conf * 100)}%`, obj.x - 50, obj.y - 65);
                ...
                if (Math.random() > 0.993) {
                    ...
                    addLog(`Detected Fruit Class: ${obj.type.toUpperCase()} · Score: ${Math.floor(conf * 100)}%`, obj.type === 'banana' ? "text-[#f5c842]" : "text-[#e84c4c]");
                }
            }
        }
    });
...
let colFruitX = 400, colFruitY = 225, colFruitDir = 1;
...
function renderCollectStudio() {
    ...
    colCtx.lineWidth = 1;
    colCtx.beginPath(); colCtx.moveTo(W / 2, 0); colCtx.lineTo(W / 2, H); colCtx.stroke();
    colCtx.beginPath(); colCtx.moveTo(0, H / 2); colCtx.lineTo(W, H / 2); colCtx.stroke();
...
function captureDataImage() {
    ...
    tempCtx.drawImage(colCanvas, colCanvas.width / 2 - 100, colCanvas.height / 2 - 100, 200, 200, 0, 0, 100, 100);

    const dataUrl = tempCanvas.toDataURL();
    collectGallery.push({ type: currentSelection, img: dataUrl });

    updateCollectGalleryUI();
    addLog(`Captured data specimen for class: ${currentSelection.toUpperCase()}`, "text-orange-400");
}
...
function updateCollectGalleryUI() {
    ...
    if (collectGallery.length === 0) {
        gal.innerHTML = '<div class="text-[10px] text-gray-500 text-center col-span-3 py-6">គ្មានរូបភាពនៅឡើយទេ</div>';
    } else {
        gal.innerHTML = collectGallery.map((item, index) => {
            if (item.type === 'banana') banCount++;
            else appCount++;

            const badgeColor = item.type === 'banana' ? 'bg-[#f5c842]/20 text-[#f5c842]' : 'bg-[#e84c4c]/20 text-[#e84c4c]';
            return `
...
function initAug() {
    ...
    grid.innerHTML = augEffects.map((eff, index) => {
...
function drawAugmentedFruit(effectId, type) {
    ...
    if (effectId === 'bright' || effectId === 'dark' || effectId === 'noise' || effectId === 'blur') {
...
function drawTradeoff() {
    ...
    const models = [
        { name: 'YOLOv8-Nano', fps: 45, map: 88, color: '#3fb950', x: 50 + (W - 100) * 0.85, y: H - 40 - (H - 80) * 0.55 },
        { name: 'YOLOv8-Small', fps: 32, map: 92, color: '#2f81f7', x: 50 + (W - 100) * 0.6, y: H - 40 - (H - 80) * 0.7 },
        { name: 'YOLOv8-Med', fps: 20, map: 95, color: '#f0883e', x: 50 + (W - 100) * 0.35, y: H - 40 - (H - 80) * 0.85 },
        { name: 'YOLOv11-Nano', fps: 25, map: 97, color: '#a371f7', x: 50 + (W - 100) * 0.45, y: H - 40 - (H - 80) * 0.94 }
    ];
...
function resetTraining() {
    ...
    document.getElementById('btnTrain').className = "px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-mono";
...
function simulateEpoch() {
    if (trainingEpoch >= 50) {
        ...
        document.getElementById('btnTrain').className = "px-4 py-1.5 bg-green-600 text-white rounded text-xs font-mono pointer-events-none";
        return;
    }
    trainingEpoch++;
    document.getElementById('currentEpoch').innerText = trainingEpoch;
    document.getElementById('trainingProgress').style.width = (trainingEpoch / 50 * 100) + '%';

    // Math logic simulating training curve progression
    const loss = 3.5 * Math.exp(-0.08 * trainingEpoch) + 0.15 + (Math.random() - 0.5) * 0.08;
    const map = 0.52 + 0.44 * (1 - Math.exp(-0.1 * trainingEpoch)) + (Math.random() - 0.5) * 0.03;
...
function drawTrainingCurves() {
    ...
    const plot = (canvas, dataList, valueKey, label, color) => {
        ...
        ctx.fillText(`${label}: ${dataList[dataList.length - 1][valueKey].toFixed(3)}`, W - 120, 20);
    };
...
function showGradCam(type) {
    ...
    document.getElementById('gcBtnBanana').className = type === 'banana' ? 'flex-1 py-2 bg-blue-600 text-white rounded text-xs font-mono' : 'flex-1 py-2 bg-white/5 text-gray-500 rounded text-xs font-mono';
    document.getElementById('gcBtnApple').className = type === 'apple' ? 'flex-1 py-2 bg-blue-600 text-white rounded text-xs font-mono' : 'flex-1 py-2 bg-white/5 text-gray-500 rounded text-xs font-mono';
    ...
    if (type === 'banana') {
        ...
        let heat = ctx.createRadialGradient(W / 2 - 20, H / 2, 10, W / 2 - 20, H / 2, 120);
...
function animateDrop(wasteObj, targetX, targetY, type) {
    ...
    const timer = setInterval(() => {
        ...
        if (step >= 1.0) {
            clearInterval(timer);
            sortedCounts[type]++;
            document.getElementById('robotPlasticSorted').innerText = sortedCounts.plastic;
...
function renderCity() {
    ...
    skyline.forEach((b, i) => {
        ...
        for (let y = b.y + 18; y < b.y + b.h - 12; y += 24) {
            for (let x = b.x + 10; x < b.x + b.w - 10; x += 18) {
                cityCtx.fillStyle = (x + y + i) % 3 === 0 ? 'rgba(245,200,66,0.7)' : 'rgba(47,129,247,0.35)';
    ...
    citySignals.forEach(signal => {
        ...
        cityCtx.fillStyle = signal.state === 'green' ? '#3fb950' : signal.state === 'amber' ? '#f5c842' : '#e84c4c';
        cityCtx.beginPath();
...
function drawLatencyProfile() {
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.font = '9px JetBrains Mono';
    ctx.fillText("Model Latency Benchmark: ~22ms/frame on INT8 Quantized model", 15, H - 10);
}
...
function clearIotLogs() {
    const logs = document.getElementById('iotLogs');
    logs.innerHTML = '<div class="text-green-400">[CONNECT] Reconnected to hivemq broker.</div>';
}
...
window.onload = () => {
    ...
    addLog("ប្រព័ន្ធសិប្បនិម្មិត FruitAI Pro ត្រូវបានកំណត់រចនាសម្ព័ន្ធរួចរាល់។", "text-green-400");

    // ━━━━━━ INITIALIZE API BACKEND CONNECTION ━━━━━━
...
function checkAPIStatus() {
    fetch(`${API_BASE_URL}/api/stats`)
        ...
        .then(data => {
            document.getElementById('apiStatus').style.background = 'var(--green)';
            updateAPIStats(data);
        })
        .catch(error => {
            console.log('API Connection Error:', error);
            document.getElementById('apiStatus').style.background = '#f97316';
        });
}

// Update live stats from backend
function updateAPIStats(stats) {
    document.getElementById('apiStatsBanana').innerText = stats.banana || '0';
    document.getElementById('apiStatsApple').innerText = stats.apple || '0';
    document.getElementById('apiStatsOrange').innerText = stats.orange || '0';
    document.getElementById('apiStatsTotal').innerText = stats.total || '0';
}

// Initialize API connection and polling
function initializeAPIConnection() {
    checkAPIStatus();
    setInterval(checkAPIStatus, 1000); // Poll every 1 second
}
